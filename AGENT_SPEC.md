...
- **Implemented (MVP):** Neon via Vercel Storage; tables `kg_nodes`, `kg_edges`, `kg_chunks` (vector 1536), `kg_patient_sync`. Sync runs on Ask when stale (>60 min). Embeddings use OpenAI `text-embedding-3-small` and are skipped for unchanged chunk text. Ask falls back to in-memory retrieval when `DATABASE_URL` or embeddings are unavailable. LLM/embed calls use `maxRetries: 0` to avoid retry burn on overload. Status/force-sync: `GET`/`POST /api/agent/graph-status`.
